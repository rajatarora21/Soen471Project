...
HR analytics project based on a classification model, depicts whether a Data Scientist is looking for a new job based on a finite set of features. We efficiently implemented a screening process and obtained significant classification metrics through the process of classification metrics through the process of analysis,cleansing and transforming the required dataset. Companies that are interested in hiring a data scientist would not only save time, but save resources to filter the potential candidates. Earlier, because of the high volume of applicants, some deserving candidates could be left out due to lack of proper screening process. Selection of the candidate depends. Moreover a data scientist is hired based on key aspects in data like experience, education, location etc. As the analysis is carried out, initially the data is cleaned by removing the null or missing data that would help in high accuracy of the prediction. As the data is later pre-processed, some columns could be dropped that are not essential to predict the requirements, that would save time and resources. As the analysis is carried forward, we classify true positives, true negative, false positives and false negatives through which we calculate accuracy and f1-score that gives the percentage of selection of the candidate. Although the analysis could save time, it can take a lot of resources to store the data. As the data set increases, it could be time consuming and acquiring more resources to clean and process the data. Not only big data sets could create problems, the prediction might not give the desired results, as the prediction is not 100% accurate. For future perspectives, as the companies implement the model, the model will get more accurate with increasing data sets. Moreover, our analysis only consists of 3 algorithms to classify, this might get better by carrying forward analysis with more algorithms that might end up calculating higher accuracy. 
